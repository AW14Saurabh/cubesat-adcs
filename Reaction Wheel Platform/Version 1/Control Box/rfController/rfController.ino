...
void loop()
{
  int switchA = digitalRead(SWITCH_A_PIN);
  ...
  float tx = switchA*(2*PI) + potAngle; //We want to send all of our data in one number, so we add 2Pi to the angle if the switch is turned on
